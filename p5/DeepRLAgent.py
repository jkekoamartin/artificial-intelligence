...
class DeepRLAgent:
    ...
    def set_next_state(self, next_observation, action, reward, terminal, test=False):
        new_state = np.append(next_observation, self.currentState[:, :, 1:], axis=2)
        self.replayMemory.append((self.currentState, action, reward, new_state, terminal))
        if len(self.replayMemory) > REPLAY_MEMORY:
            self.replayMemory.popleft()

        if self.timeStep > OBSERVE and test is not True:
            # Train the network
            self.trainQNetwork()
